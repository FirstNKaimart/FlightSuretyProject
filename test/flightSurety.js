...
contract('Flight Surety Tests', async (accounts) => {
  ...
  it('(airline) can register an Airline using registerAirline() directly without need of a consensus', async () => {
    ...
    try {
        ...
        await config.flightSuretyApp.registerAirline(config.firstAirline, "test airline 1", {from: accounts[0]});
    }
    catch(e) {
      console.log(e);
    }
  ...
  it('(airline) needs 50% votes to register an Airline using registerAirline() once there are 4 or more airlines registered)', async () => {
    ...
    try {
        await config.flightSuretyApp.registerAirline(accounts[2], "test airline 2", {from: accounts[0]});
        await config.flightSuretyApp.registerAirline(accounts[3], "test airline 3", {from: accounts[0]});
        await config.flightSuretyApp.registerAirline(accounts[4], "test airline 4", {from: accounts[0]});
    }catch (e) {
        console.log(e);
    }
  ...
  it('(airline) can register a flight using registerFlight()', async () => {
    ...
    try {
        ...
        await config.flightSuretyApp.registerFlight("BKK001", "Bangkok", flightTimestamp, {from: config.firstAirline});
    }
    catch(e) { 
      console.log(e);
    }
    let isActive = await config.flightSuretyData.isActive.call(config.firstAirline,{from: config.firstAirline})
    let result = await config.flightSuretyApp.viewFlightStatus.call("BKK001",config.firstAirline,{from: config.firstAirline});
    // ASSERT
    assert.equal(isActive, true, "Airline should be active");
    assert.equal(result, 0, "Status Code initial should be 0.");
  });
  ...
  it("(passenger) may pay up to 1 ether for purchasing flight insurance.", async () => {
    ...
    try {
        ...
        // console.log('address'+ config.firstPassenger);
        await config.flightSuretyData.buy("BKK001", {from: config.firstPassenger, value: price});
    }
    catch(e) {
      console.log(e);
    }
  ...
  it("Server will loop through all registered oracles, identify those oracles for which the OracleRequest event applies, and respond by calling into FlightSuretyApp contract with random status code", async () => {
    // ARRANGE
    let flight = 'BKK001'; // Course number
    ...
    for(let i=0; i < TEST_ORACLES_COUNT; i++) {
      let oracleIndexes = await config.flightSuretyApp.getMyIndexes({from: accounts[i]});
      for(let j=0;j<3;j++) {
        try {
          // Submit a response
          // only be accepted if there is an Index match
          await config.flightSuretyApp.submitOracleResponse(oracleIndexes[j], config.firstAirline, flight, timestamp, STATUS_CODE_LATE_AIRLINE, { from: accounts[i] });
        } catch(e) {
          // Enable this when debugging
          // console.log('\nError', j, oracleIndexes[j].toNumber(), flight, flightTimestamp);
        }
      }
    }
  ...
  it("(passenger) receives credit of 1.5X the amount they paid, if flight is delayed due to airline fault", async () => {
    // ARRANGE
    let price = await config.flightSuretyData.INSURANCE_PRICE_LIMIT.call();
    let passengerBalance = await config.flightSuretyData.getAmountToPay.call({from: config.firstPassenger}); 
    const balanceInWei = price *1.5;
    assert.equal(passengerBalance, balanceInWei, "Passenger should have 1.5 ether to withdraw.");
  });
  ...
  it("(passenger) can withdraw any funds owed to them as a result of receiving credit for insurance payout", async () => {
    ...
    let finalBalance = await config.flightSuretyData.getAmountToPay.call({from: config.firstPassenger});
    
    assert.equal(finalBalance.toString(), 0, "Passenger should have transfered the ethers to its wallet.");
    assert.equal(Number(passengerOriginalBalance) + Number(passengerBalance) - (gasPrice * gasUsed), Number(passengerFinalBalance), "Passengers balance should have increased the amount it had credited");
  });
