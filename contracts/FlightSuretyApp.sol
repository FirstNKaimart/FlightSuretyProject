...
contract FlightSuretyApp {
    ...
    */
    ...
    {
        require(flightSuretyData.isActive(msg.sender), "This airline account does not provide enough funding, airline operations are disabled.");
        _;
    }

    modifier requireAirlineNotVoted(address airlineAddress) {
        require(!checkIfExist(airlineVoters[airlineAddress]), "You already voted to register this airline.");
        _;
    }
    /********************************************************************************************/
    /*                                       UTILITY FUNCTIONS                                  */
    ...
    */   
    ...
    function checkIfExist(address[] memory _voters) internal view returns(bool Voted){
        Voted = false;
    ...
    */  
    ...
    function viewFlightStatus
                            (
                                ...
                                address _airlineAddr
                            )
                            external
                            view
                            returns(uint8)
    {
            bytes32 key = keccak256(abi.encodePacked(_flightCode, _airlineAddr));
...
interface IFlightSuretyData {
    ...
    function getAirlineVotes(address airline) external view returns (uint256 votes);
    function pay (string calldata flightCode) external;
    function withdraw (address payable insuredPassenger) external returns (uint256, uint256, uint256, uint256, address, address);
